...
set +e
wget https://ghproxy.com/https://github.com/00575/R2S/releases/download/Test-$(date +%Y-%m-%d)/$board_id$ver.img.gz -O- | gzip -dc > $board_id.img
if [ $? -eq 0 ]; then
	wget https://ghproxy.com/https://github.com/00575/R2S/releases/download/Test-$(date +%Y-%m-%d)/$board_id$ver.img.md5 -O md5sum.txt
	echo -e '\e[92m今天固件已下载，准备解压\e[0m'
else
	echo -e '\e[91m今天的固件还没更新，尝试下载昨天的固件\e[0m'
	wget https://ghproxy.com/https://github.com/00575/R2S/releases/download/Test-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.gz -O- | gzip -dc > $board_id.img
	if [ $? -eq 0 ]; then
		wget https://ghproxy.com/https://github.com/00575/R2S/releases/download/Test-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/$board_id$ver.img.md5 -O md5sum.txt
