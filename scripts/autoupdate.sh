...
wget https://github.com/00575/nanopi-R2S/releases/download/R2S-$(date +%Y-%m-%d)/R2S-$(date +%Y-%m-%d)-ROM.zip
if [ -f /mnt/mmcblk0p2/R2S*.zip ]; then
	echo -e '\e[92m今天固件已下载，准备解压\e[0m'
else
	echo -e '\e[91m今天的固件还没更新，尝试下载昨天的固件\e[0m'
	wget https://github.com/00575/nanopi-R2S/releases/download/R2S-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)/R2S-$(date -d "@$(( $(busybox date +%s) - 86400))" +%Y-%m-%d)-ROM.zip
	if [ -f /mnt/mmcblk0p2/R2S*.zip ]; then
